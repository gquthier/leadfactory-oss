...
export function validateDatabase(input, { strict = false } = {}) {
  ...
  // Une base écrite avant cette configuration (ou un export ancien) reste
  // valide : elle repart des valeurs par défaut, jamais d'une erreur.
  db.dashboard = src.dashboard === undefined ? defaultDashboard() : dashboardInput(src.dashboard);
...
export class Store {
  constructor(dataDir, { onMutation = null } = {}) {
  ...
  update(mutator) {
    const run = this.queue.then(async () => {
      ...
      this.db = draft;
      if (this.onMutation) {
        try { await this.onMutation({ state: this.read() }); }
        catch (error) { console.error('[agency] mirror failed after saving:', error?.message ?? error); }
      }
      return result;
    });
