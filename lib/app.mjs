...
import { createDashboardAccess } from './dashboard-access.mjs';
import { dashboardInput, defaultDashboard } from './dashboard-config.mjs';
...
/**
 * JSON canonique à clés triées : deux états au contenu identique produisent
 * exactement la même chaîne, quel que soit l'ordre d'insertion des propriétés.
 */
...
export async function createApp({
  ...
  // laisser derrière lui un verrou sur le dossier de données.
  const access = createDashboardAccess({ accessToken });

  const store = new Store(dataDir, { onMutation });
  await store.load();
  ...
  // Identité de ce processus : permet au navigateur de distinguer un simple
  // changement de données d'un redémarrage du serveur. Valeur aléatoire.
  const instanceId = newId();
  let revisionCache = { source: null, value: null };

  /**
   * Empreinte de l'état métier (version, agence, tableau de bord, clients,
   * campagnes, tâches, livrables) — jamais des connexions, qui contiennent la
   * clé API.
   *
   * `store.db` est remplacé par un nouvel objet à chaque mutation publiée et
   * n'est jamais muté sur place : son identité sert de cache et évite de
   * recalculer l'empreinte à chaque sondage, sans imposer de champ de version
   * au store. Un état identique redonne toujours la même empreinte, un
   * changement réel (y compris un import) en donne une autre.
   */
  function stateRevision() {
    ...
    const canonical = canonicalJson({
      ...
      // Incluse : une personnalisation du tableau de bord doit réveiller le
      // sondage des autres onglets comme n'importe quel changement de données.
    ...
    const value = createHash('sha256').update(canonical).digest('hex').slice(0, 32);
  ...
  async function handle(req, res) {
    ...
    // Seule route `/api/*` joignable sans autorisation, et seulement quand la
    // protection est active : l'échange du ticket d'ouverture contre un cookie.
    // Elle passe après checkHost/checkWriteRequest, donc une écriture d'origine
    // étrangère est déjà refusée avant d'arriver ici.
    if (access.isTicketExchange(segments, req.method)) {
      const cookie = access.redeemTicket(await readJsonBody(req));
      // Le cookie ne voyage que dans l'en-tête, jamais dans le corps JSON.
  ...
  async function api(req, res, seg, url) {
    ...
    // Métadonnées de synchronisation : aucun contenu métier, aucun secret,
    // aucun chemin local. Sert au rafraîchissement de la page ouverte.
    if (resource === 'meta' && method === 'GET' && !id) {
      return sendJson(res, 200, {
        product: 'leadfactory-oss',
        apiVersion: 1,
    ...
    // --- Tableau de bord (configuration déclarative) ---
    if (resource === 'dashboard' && !id) {
      if (method === 'GET') return sendJson(res, 200, store.read().dashboard);
      if (method === 'PUT') {
        const body = await readJsonBody(req);
        // Configuration complète validée avant toute écriture : un corps refusé
        // laisse la configuration précédente intacte sur disque.
        const next = dashboardInput(body);
        const updated = await store.update((db) => {
          db.dashboard = next;
          return db.dashboard;
        });
    ...
    if (resource === 'import' && method === 'POST') {
      ...
      // Un export produit avant cette configuration n'en contient pas : il ne
      // doit pas réinitialiser le tableau de bord en place.
      ...
      const summary = await store.update((db) => {
        ...
        db.dashboard = keepDashboard ? db.dashboard ?? defaultDashboard() : next.dashboard;
  ...
  async function close() {
    ...
    // Tickets et sessions disparaissent avec l'instance : rien ne survit.
    access.close();
    await store.close();
  }

  /**
   * Ticket d'ouverture à usage unique (64 hexadécimaux, 60 s). Destiné au seul
   * fragment `#connect=` d'une URL ouverte sur clic humain. Lève une erreur
   * explicite si cette instance n'a pas de jeton de service.
   */
  function issueDashboardTicket() {
    return access.issueTicket();
  }

  return { server, store, connections, dataDir, publicDir, close, issueDashboardTicket };
}
