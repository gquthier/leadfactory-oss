...
description: Extract brand identity from a website URL — voice, colors, typography,
  imagery, values, and target audience — into a structured brand-profile.json. The
  profile feeds downstream skills (seo-content-writer, email-composer, frontend-design,
  pro-deck-builder, cross-platform-audit) for brand-consistent output. Use when the
  user says brand DNA, brand profile, extract brand, analyze brand, brand voice, brand
  identity, brand colors, or brand style guide.
license: MIT
metadata:
  version: 1.0.0
  category: strategy
...
Construct the JSON following the schema in references/REFERENCE.md precisely. Use `null` for any field that cannot be confidently extracted — never guess.
