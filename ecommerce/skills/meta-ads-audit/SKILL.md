...
description: Meta Ads (Facebook + Instagram) account audit and business context setup.
  Run this first — it gathers business information, analyzes account health, and saves
  context that all other Meta ads skills reuse. Trigger on "audit my Meta ads", "audit
  my Facebook ads", "Meta ads audit", "set up my Meta ads", "onboard Meta", "Meta
  account overview", "how's my Meta account", "Meta health check", "what should I
  fix in my Facebook ads", or when the user is new to NotFair Meta and hasn't run
  an audit before. Also trigger proactively when other Meta ads skills detect that
  meta business-context.json is missing.
license: MIT
metadata:
  upstream: nowork-studio/toprank
---

## BizOS E-commerce workspace

The tool names and benchmark numbers inherited below describe optional upstream adapters and historical heuristics. Follow `references/shared/preamble.md` to choose a connected tool or an export; do not call missing methods. Confirm hypotheses against the current account. Preserve its currency; fields named `aov_usd` require a documented conversion, otherwise leave them unknown. Existing user authorization takes precedence over repeated confirmation wording.

Use the owner’s brief and current authorizations. First check which tools are actually available; this skill does not install or connect them. Read current official platform documentation before executing version-dependent commands. Record findings and deliverables with the `commerce_*` tools so they appear in Apps. Keep observed data, estimates and hypotheses separate. A preview, draft or prompt is not a published store, campaign or generated media file.
...
Follow `references/shared/preamble.md` — MCP detection, OAuth, ad account selection.
...
Read `references/shared/policy-registry.json`. For each entry where `last_verified + stale_after_days < today`:
...
### Diagnostic hypotheses — test against account evidence

- **Pixel + CAPI is upstream of everything.** Inspect matching quality, deduplication and event completeness together. A score alone does not establish how much performance is lost.
- **Compare attribution sources.** Reconcile Meta, storefront and analytics figures over comparable periods. Record windows, refunds and deduplication. Quantify an observed discrepancy; do not assign a universal bias or causal explanation.
- **Frequency and CPM trends.** Consider fatigue, saturation, seasonality and auction changes. Check creative-level evidence before choosing a refresh or budget action.
- **Concentration.** Describe the measured share of results and assess dependence, stability and alternatives. Concentration alone does not establish fragility or predict collapse.
- **Audience overlap.** Assess whether overlapping ad sets harm delivery or fragment measured outcomes. Compare a consolidation hypothesis with the existing objectives before proposing a test.
...
> Indiquer le chemin ou l’identifiant du livrable réellement enregistré avec `commerce_records`. Si cet enregistrement manque, le dire explicitement.
...
2. **STOP condition** — when evidence demonstrates broken conversion measurement, defer decisions that depend on those conversions until it is repaired. A missing CAPI connection or one EMQ score alone is not proof that every metric is unreliable.
...
5. **Report attribution explicitly.** Cite source, time window, conversion definition and attribution setting for ROAS. If another source differs, show the measured difference and remaining uncertainty; never invent an adjustment percentage.
