...
description: Manage Meta Ads (Facebook + Instagram) — performance, ROAS, CPM, frequency,
  ...
  ads. Use for any mention of Meta Ads, Facebook Ads, Instagram Ads, ROAS, CPM, ad
  spend, or campaign settings on Meta.
license: MIT
metadata:
...
Read and follow `references/shared/preamble.md` — handles MCP detection, OAuth, and ad account selection. Once cached, this is instant.
...
3. **Show numbers in dollars, percentages, and the right denominator.** Keep spend in the source account currency and include its ISO code; never relabel another currency as USD. Document rate, date and source for any conversion. Report CPM and CPC with their source period;, CPM and CPC always cited with the attribution window (e.g. "ROAS 3.2× on 7DC1DV"). Use **link** clicks not all-clicks for CTR. Vague metrics are not findings.
...
6. **Frequency-first triage.** Before recommending budget changes, check frequency and CPM trend. Rising frequency and CPM can suggest saturation or creative fatigue; compare audience, seasonality, delivery and creative evidence before attributing a cause.
7. **Attribution-window discipline.** Always cite the ad set's attribution setting when reporting ROAS or CPA. "ROAS 3.2×" without the window is meaningless because a different window can change the reported value; measure that difference in this account.
...
For profitability framing (Break-Even ROAS, Headroom $, MER, LTV:CAC, budget forecasting), read `references/shared/meta-math.md`.
